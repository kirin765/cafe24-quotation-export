...
- **파일 형식**: `.csv`와 `.xlsx`를 읽습니다. 구형 `.xls`는 `.xlsx`나 CSV로 저장해 주세요(안내 후 거부).
...
- .xlsx는 첫 시트만 읽고, 5MB·5,000행까지 처리합니다.
...
| 우리 열 | 인식하는 이름 |
|---|---|
| `item_code` | item_code, 상품코드, 상품 번호, 품목코드, 자체 상품코드, 자체 품목코드, 판매자 상품코드(마켓 양식), 코드 |
...
- **`재고수량`은 수량으로 쓰지 않습니다.** 재고는 '지금 있는 개수'이고 견적 수량은 '주문할 개수'라서, 자동으로 채우면 견적이 조용히 틀어집니다. 재고 열만 있으면 수량은 1로 채우고 그 사실을 경고합니다.
- 파일 인코딩은 **UTF-8과 EUC-KR을 자동으로 구분**합니다(엑셀의 'CSV(쉼표로 분리)'는 EUC-KR). 다르게 읽었으면 그 사실을 알려 줍니다.
- **마켓 양식(스마트스토어 등)도 읽습니다.** 양식 위에 제목·안내 행이 있으면 앞 10행에서 열 이름 행을 찾아 건너뛰고, `필수/비필수` 안내 행과 `…입력할 수 있습니다` 설명 행은 상품이 아니므로 건너뜁니다. 건너뛴 행 수는 화면에 알려 줍니다.
- 마켓 양식에 딸린 **예시 상품 행**은 실제 데이터처럼 보이므로 자동으로 지우지 않습니다. 올리기 전에 지우거나, 가져온 뒤 표에서 삭제하세요.
- 200행 제한은 안내 행을 뺀 **상품 행** 기준입니다. 건너뛴 안내 행 번호를 경고에 함께 보여 주므로, 상품 행이 잘못 건너뛰어졌는지 바로 확인할 수 있습니다.
- 마켓 양식의 **조합형 옵션**은 한 칸에 여러 값이 들어 있습니다(예: `빨강,노랑`). 한 행을 한 상품으로 가져오고 옵션 칸 내용을 그대로 두므로, 필요한 옵션만 남기고 정리하세요. **옵션 추가금액(옵션가)은 반영하지 않습니다** — 옵션마다 단가가 다르면 표에서 조정하세요.
...
| 항목 | 내용 |
|---|---|
| 선택(쓰기) | `xlsx` (SheetJS Community Edition) **0.18.5 고정** (`^` 없이 핀) |
| 라이선스 | Apache-2.0 |
...
- 고객이 올린 .xlsx는 직접 만든 최소 파서로 값만 읽습니다(5MB·5,000행·60,000셀 제한). 매크로·수식은 실행하지 않으며, 값은 텍스트로만 씁니다.
