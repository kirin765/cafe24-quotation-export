...
```text
npm test           80 passed + 12 skipped (DATABASE_URL 없을 때 통합 테스트는 건너뜀)
DB 통합 테스트      12/12 (실제 Neon, 합성 몰 ID 사용 후 정리)
npm run typecheck  통과
npm run lint       통과 (오류 0)
npm run build      통과 (정적 4개 + 동적 페이지·API 15개)
브라우저 확인       14/14 (데모), PDF 8/8, 초안 흐름 25/25, 실제 형식 12/12, 라우트 20/20 + 40/40
```
...
- **실제 운영자의 품목표**로 변환·대조 (아래 5차는 웹에서 확인한 표준 양식으로 만든 대체 검증이다)
